...
class DeviceHealthTest extends TestCase
{
    ...
    protected DeviceHealthService $service;

    protected function setUp(): void
    {
        parent::setUp();
        $this->service = $this->app->make(DeviceHealthService::class);
    }
    ...
    /**
     * Test service creates health object for door.
     */
    public function test_health_service_creates_health_for_door(): void
    {
        $door = Door::create([
            'door_id' => 'DOOR-TEST',
            'name' => 'Test Door',
        ...
        // Mock the ISAPI service to avoid live device query
        $this->mock(\App\Services\HikvisionIsapiService::class, function ($mock) {
            $mock->shouldReceive('getDeviceStatus')
                ->andReturn([
                    'model' => 'DS-K1T804AMF',
                    'serial' => 'TEST-001',
                    'firmware' => 'v2.3.0',
        ...
        $service = $this->app->make(DeviceHealthService::class);
        $health = $service->getHealthForDoor($door);
        
        $this->assertInstanceOf(DeviceHealth::class, $health);
        $this->assertEquals('online', $health->status);
        $this->assertEquals(96, $health->user_count);
        $this->assertEquals(82, $health->card_count);
    }
}
